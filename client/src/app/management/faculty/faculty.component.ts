...
export class FacultyComponent implements OnInit{
  ...
  constructor(private router:Router, private http: HttpClient) {
    ...
    this.faculty_id = this.userData.id;
  }
  ...
  getPrograms() {
    this.http.get('http://127.0.0.1:8000/api/program/display-program' + '/' + this.faculty_id).subscribe((resultData:any) => {
  ...
  viewProgramDetails(data:any) {
    this.viewModal = true;
    this.http.get('http://127.0.0.1:8000/api/program/display-program-title' + '/' + data.id).subscribe((resultData:any) => {
